...
document.getElementById('category').onchange = function () {
  ...
  for (var i = 0; i < array[changedCategory].length; i++) {
...
var preview = document.getElementsByClassName('preview'); // input[type=file]の取得

var fileInput = document.getElementsByClassName('uploadfile'); //inputタグ含む文字等の取得

var inputArea = document.getElementsByClassName('inputArea'); //画像表示エリアの取得

var previewBox = document.getElementsByClassName("previewBox"); //画像削除ボタンの取得

var removeImg = document.getElementsByClassName('removeImg'); //画像アップロードから削除までのループ
...
var _loop = function _loop(i) {
  ...
  preview[i].addEventListener('drop', function (e) {
    e.preventDefault();
    this.classList.remove('dragover');
    var files = e.dataTransfer.files; // 取得したファイルをinput[type=file]へ
    ...
    reader.onload = function (event) {
      ...
      img.setAttribute("class", "previewImage h-full");
      previewBox[i].appendChild(img);
    };

    reader.readAsDataURL(files); //削除ボタンの表示
  ...
  preview[i].addEventListener('click', function () {
    if (document.createEvent) {
      var evt = document.createEvent('MouseEvents'); // イベントの初期化

      evt.initEvent('click', false, true); // イベントを発生させる
...
searchForm.addEventListener('keyup', function () {
  ...
  itemList.forEach(function (elm) {
    ...
    var val = elm.dataset.set;
    var word = new RegExp(keyWord, "i");
...
for (var _i = 0; _i < items.length; _i++) {
  items[_i].addEventListener('click', function (e) {
