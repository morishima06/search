...
<section class="md:flex flex-1">
            ...
            <form action="{{route('category',$category)}}" method="get">
                ...
                <div class="accodion_box w-full">
                    <ul class="px-3">
                        ...
                        <div class="accordion1  border-slate-300  border-y">
                            ...
                            <div class="accordion-content px-5 pt-0  overflow-hidden max-h-0">
                                <div class="leading-6 font-light  text-justify mb-3">
                                    <div><a class="select {{ $category == 'wear' ? 'font-bold	' : '' }}" href="{{route('category', ['category' => 'wear','min' => $min,'max' => $max])}}"> ウェア</a></div>
                                    <div><a class="select {{ $category == 'shoes' ? 'font-bold	' : '' }}" href="{{route('category', ['category' => 'shoes','min' => $min,'max' => $max])}}"> シューズ</a></div>
                                    <div><a class="select {{ $category == 'accessories' ? 'font-bold	' : '' }}" href="{{route('category', ['category' => 'accessories','min' => $min,'max' => $max])}}"> アクセサリー</a></div>
                                    <div><a class="select {{ $category == 'lifestyle' ? 'font-bold	' : '' }}" href="{{route('category', ['category' => 'lifestyle','min' => $min,'max' => $max])}}"> ライフスタイル</a></div>
                                </div>
                            </div>
                        </div>
                        <!-- accordion2 -->
                        ...
                        <!-- accordion3 -->
                        <div class="accordion3 border-slate-300 w-full border-b">
                            <!-- header -->
                            <div class="accordion-header flex justify-between  cursor-pointer border-gray-300  py-3">

                                <li class="text-gray-700 font-semibold">ブランド</li>
                                <span class="mt-1 border-gray-700 border-solid border-r-2 border-b-2 w-3 h-3 mr-3 rotate-45  inline-block "></span>
                            </div>
                            <!-- Content -->
                                ...
                                <div class="w-full max-h-[300px] overflow-scroll">
                                    ...
                                    <div class="">
                                        <input type="checkbox" id="{{$group_brand->brand_name}}" name="brand_name[]" class=" ml-2 my-0.5" <?php if (isset($brand_name)) {
                                                                                                                                                if (in_array($group_brand->brand_name, $brand_name)) {
                                                                                                                                                    echo 'checked';
                                                                                                                                                }
                        ...
                        <div class="accordion4 border-slate-300 w-full border-b">
                            ...
                            <div class="accordion-content  pt-0 overflow-hidden max-h-0">
                                ...
                                <div class="w-full ">
                                    ...
                                    <div>
                                        <input type="checkbox" id="{{$group_color->color}}" name="color[]" class="ml-2 my-0.5" <?php if (isset($color)) {
                                                                                                                                    if (in_array($group_color->color, $color)) {
                                                                                                                                        echo 'checked';
                                                                                                                                    }
    ...
    <form action="{{route('category',$category)}}" method="get" class="selectForm">
        ...
        <div class="   md:w-60  lg:w-full  hidden md:inline-block p-5	">
            <div class="accordion_box ">
                ...
                <!-- accordion3 -->
                <div class="accordion3  border-b border-slate-300  ">
                    <!-- header -->
                    <div class="accordion-header cursor-pointer transition flex   justify-between items-center h-9 my-1 ">

                        <h3 class="text-slate-500 font-semibold text-1xl">ブランド</h3>
                        <i class="fa fa-angle-down arrow_b" aria-hidden="true"></i>
                    </div>
                    <!-- Content -->
                    <div class="accordion-content  pt-0 overflow-hidden max-h-0">
                ...
                <div class="accordion4  border-b border-slate-300  ">
                    ...
                    <div class="accordion-content  pt-0 overflow-hidden max-h-0">
                        ...
                        <div class="w-full ">
                            @foreach($group_colors as $group_color)
                            <div>
                                <input type="checkbox" id="{{$group_color->color}}" name="color[]" class="selector ml-2 my-0.5" <?php if (isset($color)) {
                                                                                                                                    if (in_array($group_color->color, $color)) {
                                                                                                                                        echo 'checked';
                                                                                                                                    }
                                                                                                                                } ?> value="{{$group_color->color}}">
            ...
            <ul class="flex flex-wrap">
                ...
                <div class="w-1/3  lg:w-1/4 relative border-box border  border-slate-100">
                    ...
                    <div class="w-full px-2  pb-1">
                        <p class="font-semibold	text-sm sm:text-lg break-all">{{$product->brand_name}}</p>
                        <p class="text-xs sm:text-sm line-clamp-1 break-all">{{$product->product_name}}</p>
